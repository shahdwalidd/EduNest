...
const LeaderBoard: FC = () => {
  ...
  const remainingStudents = allStudents.slice(3).filter(student =>
    student.studentFullName.toLowerCase().includes(searchTerm.toLowerCase())
  ); 

  // Helper for Podium UI - standardized fonts
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const renderPodiumCard = (student: any, rank: number, isWinner = false) => (
    <div className={`relative flex flex-col items-center text-center transition-all duration-500 w-full max-w-[320px] ${
      isWinner 
        ? 'bg-gradient-to-br from-slate-900 to-slate-950 text-white rounded-3xl border-4 border-[var(--primary-500)] shadow-2xl scale-105 p-8 z-20 md:-mb-6' 
        : 'bg-white text-slate-900 rounded-3xl border border-slate-100 shadow-xl p-8 z-10'
    }`}>
      <div className="flex flex-col items-center mb-6 relative">
        <div className={`w-10 h-10 flex items-center justify-center rounded-full text-sm font-bold mb-3   absolute  -top-14 ${
          isWinner ? 'bg-yellow-400 text-gray-600' : 'bg-slate-100 text-slate-600'
        }`}>
          {isWinner ? (
         <Medal className='text-gray-600 ' />
) : ( 
            <span className="text-sm font-bold">#{rank}</span>
          )}
        </div>
        <div className={`h-28 w-28 overflow-hidden rounded-2xl ring-4 ${
      ...
      <div className="mb-6">
        <h3 className={`text-xl font-bold leading-tight ${isWinner ? 'text-white' : 'text-slate-900'}`}>
      ...
      <div className={`grid grid-cols-2 gap-4 pt-6 border-t w-full ${
        ...
        <div>
          <p className="text-2xl font-bold">{student.completedSessions ?? 0}</p>
      ...
      <div className=" mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 border-b border-slate-100 pb-10">
          <div>
            ...
            <h2 className="text-3xl font-semibold text-slate-900 tracking-tight mt-2">Scholarly Excellence Leaderboard</h2>
          </div>
          <div className="flex gap-4">
            <div className="bg-white rounded-3xl border border-slate-100 px-6 py-4 text-center shadow-sm min-w-[130px]">
              <p className="text-2xl font-bold text-slate-900">{isLoading ? '—' : data?.totalStudents ?? '0'}</p>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Total Scholars</p>
            </div>
            <div className="bg-slate-950 rounded-3xl px-6 py-4 text-center shadow-xl min-w-[130px]">
              <p className="text-2xl font-bold text-white">#{isLoading ? '—' : data?.userRank ?? '-'}</p>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Your Rank</p>
            </div>
          </div>
        </div> 

        {/* Podium */}
        <div className="flex flex-col-reverse md:flex-row items-center md:items-end justify-center gap-6 pt-6">
          {isLoading ? (
            ...
            <>
              ...
              {topThree[0] && renderPodiumCard(topThree[0], 1, true)}
              ...
              <table className="min-w-full border-separate border-spacing-0">
                <thead>
                  <tr className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                    ...
                    <th className="px-6 py-4 text-left font-bold">Scholar</th>
                    <th className="px-6 py-4 text-left font-bold">Sessions</th>
                    <th className="px-6 py-4 text-left font-bold">Achievements</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {remainingStudents.map((student) => (
                    <LeaderboardRow 
                      ...
                      student={{ ...student, rank: allStudents.findIndex(s => s.studentEmail === student.studentEmail) + 4 + 1 }} 
