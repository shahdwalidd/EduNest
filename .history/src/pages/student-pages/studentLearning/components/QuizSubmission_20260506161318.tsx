    ...
    const fetchData = async () => {
      try {
        const results = await Promise.allSettled([
          ...
          getStudentQuizScore(quizId),
        ]);

        // Handle quiz details
        if (results[0].status === 'fulfilled') {
          setQuiz(results[0].value);
          setTimeLeft(results[0].value.durationMinutes * 60);
        } else {
        ...
        if (results[1].status === 'fulfilled') {
          setQuestions(results[1].value);
        } else {
          throw new Error('Failed to load quiz questions');
        }

        // Handle student score (optional - won't fail the entire load)
        if (results[2].status === 'fulfilled' && results[2].value) {
          const studentScore = results[2].value;
          if (studentScore.isSubmitted) {
