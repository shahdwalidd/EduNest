            ...
            {selectedItem && selectedWeek ? (
              <div className="space-y-6">
                ...
                {(() => {
                  ...
                  return (
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-3xl p-8 shadow-xl border border-blue-100">
                      <div className="flex items-center gap-4 mb-6">
                        <div className="w-12 h-12 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center">
                          <BookOpen className="w-6 h-6 text-white" />
                        </div>
                        <div>
                          <h3 className="text-2xl font-bold text-slate-900">Quiz Time!</h3>
                          <p className="text-slate-600">{selectedItem.completed ? 'Completed' : 'Ready to test your knowledge'}</p>
                        </div>
                      </div>
                      <div className="bg-white rounded-2xl p-6 shadow-sm">
                        <p className="text-slate-600 mb-6">Quiz implementation coming soon. This supports the new QUIZ content type from backend.</p>
                        <button 
                          disabled={selectedItem.completed}
                          className="bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-8 py-3 rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                        >
                          {selectedItem.completed ? '✓ Completed' : 'Start Quiz'}
                        </button>
                      </div>
                    </div>
                  );
