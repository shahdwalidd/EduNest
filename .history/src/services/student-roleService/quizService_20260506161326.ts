...
export const getStudentQuizScore = async (quizId: number): Promise<StudentQuizScore | null> => {
  try {
    ...
    // If 404 or not found, student hasn't submitted yet - return null (not an error)
    if (err.response?.status === 404) {
      return null;
    }
    // For other errors, log but don't throw - this shouldn't block quiz loading
    console.warn('Warning: Could not fetch student quiz score:', err.message);
    return null;
  }
};