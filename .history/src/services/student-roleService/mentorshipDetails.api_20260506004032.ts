...
// Map beforeEnroll details (public view)
const mapBeforeEnrollDetails = (payload: Record<string, unknown>): MentorshipDetails => {
...
// Map afterEnroll details (enrolled view)
const mapAfterEnrollDetails = (payload: Record<string, unknown>): MentorshipDetails => {
  const mentor = (payload.mentor ?? {}) as Record<string, unknown>;
  const tags = Array.isArray(payload.tags) ? payload.tags.map(String) : [];
  const whatWillLearn = Array.isArray(payload.whatWillLearn) ? payload.whatWillLearn.map(String) : [];
  ...
  // topMentorMentorships not in afterEnroll, set empty
  const topMentorMentorships: MentorshipSummary[] = [];
...
export const fetchMentorshipDetails = async (
  ...
  // Prefer beforeEnroll for public view, fallback to afterEnroll for enrolled view
  if (mentorshipData?.beforeEnroll && Object.keys(mentorshipData.beforeEnroll).length > 0) {
    payload = mentorshipData.beforeEnroll as Record<string, unknown>;
  } else if (mentorshipData?.afterEnroll && Object.keys(mentorshipData.afterEnroll).length > 0) {
    payload = mentorshipData.afterEnroll as Record<string, unknown>;
  }
  ...
  return mapBeforeEnrollDetails(payload); // Reuse before mapper for core fields; works for both
};
